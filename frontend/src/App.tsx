...
import type { Approval, AuditEvent, Budget, CapturePreview, CookBrief, CookProfile, Dish, DiscoveredDish, DishHistory, GoogleCalendarConnection, HealthResponse, Household, InventoryLot, Leftover, MealLoop, Member, PlanResponse, Preference } from "./api/contracts";
import "./styles.css";
...
export default function App() {
  ...
  const load = async (selected?: number | null) => {
    try {
      ...
      if (id) {
        ...
        setMembers(nextMembers); setMemberCalendars(Object.fromEntries(calendars)); setProfile(nextProfile); setBudget(nextBudget); setInventory(nextInventory); setLeftovers(nextLeftovers); setDishes(nextDishes); setHistory(nextHistory); setPreferences(nextPreferences); setLoops(nextLoops); setAudit(nextAudit); setApprovals(nextApprovals);
      }
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); const calendarMessage = params.get("calendar_connection") === "success" ? "Google Calendar connected. Choose the calendar for this member below." : params.get("calendar_error"); if (calendarMessage) window.history.replaceState({}, "", window.location.pathname); void load().then(() => { if (calendarMessage) setMessage(calendarMessage); }); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return <main>
    ...
    {requireHousehold() && screen === "household" && <HouseholdPage household={current!} members={members} calendars={memberCalendars} profile={profile} budget={budget} onRefresh={refresh} />}
...
function HouseholdPage({ household, members, calendars, profile, budget, onRefresh }: { household: Household; members: Member[]; calendars: Record<number, GoogleCalendarConnection>; profile: CookProfile | null; budget: Budget | null; onRefresh: () => void }) {
