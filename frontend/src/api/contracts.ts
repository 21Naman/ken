...
export interface Recommendation { dish: string; score: number; prep_minutes: number; missing_ingredients: IngredientGap[]; procurement: { route: string; estimated_cost_inr: number; items: Array<IngredientGap & { store: string | null; price_inr: number | null }>; reason: string }; score_explanation: { expiry_use: string[]; leftover_use: string[]; novelty: string; gaps: number }; }
