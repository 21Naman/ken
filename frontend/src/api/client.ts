import type { Approval, AuditEvent, Budget, CapturePreview, CookBrief, CookProfile, Dish, DiscoveredDish, DishHistory, GoogleCalendarConnection, GoogleCalendarEvent, GoogleCalendarOption, HealthResponse, Household, InventoryLot, Leftover, MealLoop, Member, PlanResponse, Preference, ZeptoCart, ZeptoStatus } from "./contracts";
...
export const api = {
  ...
  googleCalendar: (id: number, member: number) => request<GoogleCalendarConnection>(`${householdPath(id)}/members/${member}/google-calendar`), googleCalendarConnect: (id: number, member: number) => request<{ authorization_url: string }>(`${householdPath(id)}/members/${member}/google-calendar/connect`, { method: "POST" }), googleCalendars: (id: number, member: number) => request<GoogleCalendarOption[]>(`${householdPath(id)}/members/${member}/google-calendar/calendars`), selectGoogleCalendar: (id: number, member: number, body: { calendar_id: string; calendar_name: string }) => request<GoogleCalendarConnection>(`${householdPath(id)}/members/${member}/google-calendar`, { method: "PUT", body: JSON.stringify(body) }), disconnectGoogleCalendar: (id: number, member: number) => request<void>(`${householdPath(id)}/members/${member}/google-calendar`, { method: "DELETE" }),
  googleCalendarEvents: (id: number, member: number, start: string, end: string) => request<GoogleCalendarEvent[]>(`${householdPath(id)}/members/${member}/google-calendar/events?start=${start}&end=${end}`),
  cookBrief: (id: number, planDate: string) => request<CookBrief>(`${householdPath(id)}/schedule-cook-brief`, { method: "POST", body: JSON.stringify({ plan_date: planDate }) }),
  ...
  zeptoStatus: (id: number) => request<ZeptoStatus>(`${householdPath(id)}/zepto/status`), zeptoConnect: (id: number) => request<{ authorization_url: string }>(`${householdPath(id)}/zepto/connect`, { method: "POST" }), zeptoCart: (id: number, body: object) => request<ZeptoCart>(`${householdPath(id)}/zepto/cart`, { method: "POST", body: JSON.stringify(body) }), zeptoSyncInventory: (id: number, items: object[]) => request(`${householdPath(id)}/zepto/sync-inventory`, { method: "POST", body: JSON.stringify({ items_added: items }) }),
